...
Driver for aorsa/cql3d iteration with logic exposed here rather than buried in the 
...
# ------------------------------------------------------------------------------
    ...
    def step(self, timestamp=0):
        ...
        if sim_mode == 'NORMAL' :   # i.e. not RESTART do Maxwellian runs
            ...
            try:
                services.call(fpComp, 'step', t,rf_code='aorsa',pwrscale_on=pwrscale_on)
            except Exception:
            ...
            try:
                services.call(rf_icComp, 'step', t, \
                              save_output = 'False', pwrscale_on=pwrscale_on)
            except Exception:
                message = 'RF_IC aorsa mode step failed'
                print(message)
                services.exception(message)
                raise
...
# ------------------------------------------------------------------------------
    ...
    def checkpoint(self, timestamp=0.0):
        print('aorsa_driver.checkpoint() called')
# ------------------------------------------------------------------------------
#
# finalize function 
#
