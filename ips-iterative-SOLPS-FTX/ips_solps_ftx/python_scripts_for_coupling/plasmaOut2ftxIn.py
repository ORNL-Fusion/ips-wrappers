...
def plasmaOut2ftxIn(plasmaOutFile='plasmaOut.txt', ftxInFile='ftxIn.txt'):
    ...
    print(' ')
    print('from plasmaOut2ftxIn, called with input file')
    print(plasmaOutFile)
    ...
    print('\t TEST: solps output dict = ', solpsParams)
    sys.stdout.flush()
    ...
    if ('flux' in solpsParams):
        ...
        print('\t TEST: ftx[flux] = ', ftxInputs['flux'])
    sys.stdout.flush()
    ...
    if 'heatFlux' in solpsParams:
        ...
        print('\t \t TEST: heat flux tempHandler =', ftxInputs['tempHandler'], 'and temparam =', ftxInputs['tempParam'])
        print('\n')
    sys.stdout.flush()
    ...
    for n in range(len(ftxInputs['plasmaSpecies'])):
        ...
        if (n==0):
            ...
            print('\t \t TEST: as of loop for ', s, 'ftxInputs[fluxFraction] = ', ftxInputs['fluxFraction'])
            ...
            if  (('inputEnergy' in solpsParams) or ('Ti' in solpsParams) or ('Te' in solpsParams)):
                ...
                print('\t \t TEST: as of loop for ', s, 'ftxInputs[inputEnergy] = ', ftxInputs['inputEnergy'])
            ...
            if (('inputAngle' in solpsParams) or ('bfieldAngle' in solpsParams)):
                ...
                print('\t \t TEST: as of loop for ', s, 'ftxInputs[inputAngle] = ', ftxInputs['inputAngle'])
            sys.stdout.flush()
            print('\n')
        ...
        #else:
        #    #always write flux fraction, even if zero:
        #    ftxInputs['fluxFraction'].append(0.0)
        #    print('TEST: as of loop for ', s, 'ftxInputs[fluxFraction] = ', ftxInputs['fluxFraction'])
        #    #write inputEnergy if input provided by SOLPS:
        #    if  (('inputEnergy' in solpsParams) or ('Ti' in solpsParams) or ('Te' in solpsParams)):
        #        ftxInputs['inputEnergy'].append(0.0)
        #        print('TEST: as of loop for ', s, 'ftxInputs[inputEnergy] = ', ftxInputs['inputEnergy'])
        #    #write inputAngle if input provided by SOLPS:
        #    if (('inputAngle' in solpsParams) or ('bfieldAngle' in solpsParams)):
        #        ftxInputs['inputAngle'].append(0.0)
        #        print('TEST: as of loop for ', s, 'ftxInputs[inputAngle] = ', ftxInputs['inputAngle'])
        #    sys.stdout.flush()
	    ...
	    #inputEnergy
            ...
            if  ('inputEnergy' in solpsParams):
                print('\t \t TEST: solpsParams[inputEnergy][m] = ', Ein[m]) # solpsParams['inputEnergy'][m]) DEL
                ...
                if ('Te' in solpsParams):
                    if ('Z' in solpsParams):
                        ...
                        print('\t \t TEST: solpsParams[Ti] = ', Ti[m]) #solpsParams['Ti'][m]) DEL
                        print('\t \t TEST: solpsParams[Z] = ', solpsParams['Z'][m]) #may need to change to Z[m]
                        print('\t \t TEST: solpsParams[Te] = ', Te[m]) #solpsParams['Te'][m]) DEL
                ...
                if ('Z' in solpsParams):
                    ...
                    print('\t \t TEST: solpsParams[Te] = ', solpsParams['Te'][m])
                    sys.stdout.flush() #TEST
