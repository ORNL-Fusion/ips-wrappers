...
# 6/16/2014 DBB
# About zero ECRF power:  We don't want to run an ECRF code when the RF power is zero.
# The STEP function needs to produce a partial
# plasma state only containing the ECRF data (i.e. ps_write_update_file not
# ps_store_plasma_state).  This really has to be done with plasma state fortran code not
# with the python netcdf interface.So I wrote a simple code called zero_RF_EC_power to set
# all EC source profiles in plasma state to zero and then write a partial plasma state.
...
# Check if ECRF power is zero (or effectively zero).  If true don't run GENRAY just
# run zero_RF_EC_power fortran code
        ...
        if(sum(power_ec) < 0.001):
            ...
            # N.B. zero_RF_EC_power does not produce a complete set of GENTAY output
    ...
    #     Launch genray - N.B: Path to executable is in config parameter GENRAY_BIN
            ...
            if (retcode != 0):
                logMsg = 'Error executing command: ', self.GENRAY_BIN
