...
# ------------------------------------------------------------------------------
    ...
    def init(self, timeStamp=0):
        print 'py model_NB_2_mcmd.init() called'
...
# Copy current state files and input files over to working directory
        ...
        NB_bin = os.path.join(self.BIN_PATH, 'py_model_NB_2_mcmd')

        print 'Executing ', [NB_bin, cur_state_file, 'INIT', timeStamp]
        retcode = subprocess.call([NB_bin, cur_state_file])
        if (retcode != 0):
...
# Update (original) plasma state
        ...
        retcode = subprocess.call(['touch', 'NB.out'])
...
#       cur_jsdsk_file, 'STEP', timeStamp])
        task_id = services.launch_task(self.NPROC, cwd, NB_bin)
...
## "Archive" output files in history directory
        retcode = subprocess.call(['touch', 'NB.out'])
        print 'return code for touch NB.out ', retcode
        services.stage_output_files(timeStamp, self.OUTPUT_FILES)
        return partial_file
