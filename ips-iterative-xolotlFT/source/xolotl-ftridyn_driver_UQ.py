			...
			while self.xolotlExitStatus=='collapsed' or self.xolotlExitStatus=='overgrid':
				...
				if self.collapsedLoops<=int(self.driver['MAX_COLLAPSE_LOOPS']):
					...
					if self.xolotlExitStatus=='good':
						...
						shutil.copyfile('xolotlStop.h5', self.xp.parameters['networkFile']) # do we need to copy the network file here?
						transferGrid.transferGrid(self.xp.parameters['networkFile'])
						print('\t DONE RUNNING transferGrid!')
						## NOTE: Assume changes in IPS config file are up to the user
						## (this is also how the time steps are handled, I think)
