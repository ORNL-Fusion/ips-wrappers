...
class xolotlFtridynDriver(Component):
    ...
    def init(self, timeStamp=0.0):
        ...
        print 'this is a test: read from ips.config GITR_INPUT_PARAMETERS[plasmaSpecies] =', self.GITR_INPUT_PARAMETERS['plasmaSpecies']
        print 'this is a test: the elements of this ', type(self.GITR_INPUT_PARAMETERS['plasmaSpecies']), 'are '

        for i in  range(len(self.GITR_INPUT_PARAMETERS['plasmaSpecies'])):
            print 'this is a test: element ', i, ' is ', self.GITR_INPUT_PARAMETERS['plasmaSpecies'][i]
        

        self.plasmaSpecies=self.GITR_INPUT_PARAMETERS['plasmaSpecies']
        print 'this is a test: coplies to driver variable self.plasmaSpecies = ', self.plasmaSpecies
        print 'this is a test: the elements of this ', type(self.plasmaSpecies), 'are '

        for i in  range(len(self.plasmaSpecies)):
            print 'this is a test: element ', i, ' is ', self.plasmaSpecies[i]



        #### FTRIDYN PARAMETERS ##### 
        ##TURN INTO COMPONENT-INDEP LOOP #########
        print '\n'
        ...
        #for i in range(len(self.plasmaSpecies)):
            ...
            print 'this is a test: loop for species ', prj
            
            print 'this is a test: input energy to be assigned ', inputEnergy[i], ' of type ', type(inputEnergy[i])
        
            #self.energyIn[i] = float(self.ftridyn['inputEnergy'][i])
            self.energyIn.append(float(inputEnergy[i]))
            print 'this is a test: input energy assigned ', self.energyIn[i] , ' of type ', type(self.energyIn[i])
            ...
            print '\n'
            #print 'this is a test: read GITR parameter fluxFraction for ', prj, ' with value ' ,self.fluxFraction[i]
            ...
            #lay_file=self.FTX_LAY_FILE[i]
            #print 'this is a test: LAY file', lay_file
            #self.other_ft_input_files[i]=[self.FT_SURFACE_FILE,self.FTX_LAY_FILE[i]]#self.FT_OTHER_INPUT_FILES_He
    ...
    def step(self, timeStamp=0.0):
        ...
        while time<self.driver['END_TIME']:
            ...
            if (self.driverMode == 'INIT'):
                ...
                print('\t init mode no')
                self.ftridyn['iQ0']=-1
                print 'this is a test: totalDepth = ', self.ftridyn['totalDepth'], ' of type ' , type(self.ftridyn['totalDepth'])
                print 'this is a test: too many indexes??', translate_xolotl_to_ftridyn.xolotlToLay(totalDepth=self.ftridyn['totalDepth'])
                ...
                for prj in self.plasmaSpecies:
                    if prj!='He': #do not self-copy
                        print 'this is a test: copy ', self.FTX_LAY_FILE[0], ' as ', self.FTX_LAY_FILE[i]
            ...
            i=0 ; #keep track of which projectile, as energy, angle, etc. as given as array, e.g.: [E(He), E(W), E(D), E(T)]
            ...
            spYString=str(time)
            for prj in self.plasmaSpecies:                
                print 'write sputtering Yield due to ', prj , ' = ', self.spYield[i], 'to file'
            ...
            i=0
            for prj in self.plasmaSpecies:
                #if self.fluxFraction[i]>0.0: #DELETE WHEN D and T ARE IN XOLOTL
                ft_output_profile_temp_prj=timeFolder+'/'+self.FT_OUTPUT_PROFILE_TEMP+'_'+prj
                print 'this is a test: reading from ', ft_output_profile_temp_prj
                profile=open(ft_output_profile_temp_prj, "r")
                tridynString=profile.read().rstrip('\n')
                print '\t with content ', str(tridynString)
                ...
                profile.close()
                #else:
                #print 'skip profiles of fluxFraction<0.0 until D and T implemented in Xolotl'
                i+=1            
            combinedFile.close()

            #see if Xolotl works with an old tridyn.dat file
            #shutil.copyfile('../../tridyn_copy.dat','tridyn.dat')
            ...
            statinfo = os.stat('last_TRIDYN_toBin.dat')
            print 'prior re-binning last_TRIDYN_toBin.dat length = ', statinfo.st_size
            binTRIDYN.binTridyn()
            statinfo = os.stat('last_TRIDYN.dat')
            print 'after re-binning last_TRIDYN.dat length = ', statinfo.st_size
