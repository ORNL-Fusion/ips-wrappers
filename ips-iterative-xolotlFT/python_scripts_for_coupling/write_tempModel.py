...
def v1(xp_parameters={},plasma={}):
    print('running write_tempModel for Xolotl v1')

    print('\t TEST: with inputs:')
    print('\t dictionary xp_parameters ') # include if TEST , xp_parameters)
    print('\t dictionary plasma ') # include if TEST , plasma)
    print(' ')
    sys.stdout.flush()
    ...
    if ('heat' in plasma) or ('heat' in xp_parameters) or ('startTemp' in plasma) or ('startTemp' in xp_parameters):
        if 'heat' in plasma:
            ...
            print('\t no heat or temperature defined in PLASMAs output or Xolotls input')
