...
class sim_params:
	...
	def print_input_file(self):
		...
		f = open(self.name+self.sim_num+'.IN', "r")
...
def Prj_Tg_xolotl(sim_number=1, number_histories=1000000,
				  incident_energy=200.0, incident_angle=0.0, fractal_dimension=1.0,
				  IQ0=0, number_layers=100, depth=200.0, projectile_name='He',target1_name='W', target2_name='',target3_name='',target4_name='', ED_He='', EF_He='', E0_He='', ALPHA0_He='', ED_W='', EF_W='', SBV_W=''):
		...
		if ED_He:
			species_list[0].ED = ED_He
			print("Updated species_list[0].ED from", species_list[0].ED, "to", ED_He)
		if EF_He:
			species_list[0].EF = EF_He
			print("Updated species_list[0].EF from", species_list[0].EF, "to", EF_He)
		if E0_He:
			species_list[0].E0 = E0_He
			print("Updated species_list[0].E0 from", species_list[0].E0, "to", E0_He)
		if ALPHA0_He:
			species_list[0].ALPHA0 = ALPHA0_He
			print("Updated species_list[0].ALPHA0 from", species_list[0].ALPHA0, "to", ALPHA0_He)
		if ED_W:
			species_list[1].ED = ED_W
			print("Updated species_list[1].ED from", species_list[1].ED, "to", ED_He)
		if EF_W:
			species_list[1].EF = EF_W
			print("Updated species_list[1].ED from", species_list[1].ED, "to", ED_He)
		if SBV_W:
			species_list[1].SBV = SBV_W
			print("Updated species_list[1].SBV from", species_list[1].SBV, "to", SBV_W)
