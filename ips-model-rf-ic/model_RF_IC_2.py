...
# Copy current state files and input files over to working directory
        ...
        RF_IC_bin = os.path.join(self.BIN_PATH, 'model_RF_IC_2_mcmd')
...
# ------------------------------------------------------------------------------
    ...
    def step(self, timeStamp):
        print 'model_RF_IC_2.step() called'
...
# Call model_RF_IC
        RF_IC_bin = os.path.join(self.BIN_PATH, 'model_RF_IC_2')

        print 'Executing ', [RF_IC_bin, cur_state_file, 'STEP', timeStamp]
        task_id = services.launch_task(self.NPROC, pwd, RF_IC_bin, cur_state_file,
             cur_eqdsk_file, cur_cql_file, cur_dql_file, 'STEP', timeStamp)
