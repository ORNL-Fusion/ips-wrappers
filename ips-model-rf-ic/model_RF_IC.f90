...
!         Eliminated a large number of previous versions of this component and reverted 
!         name from model_RF_IC_3 to model_RF_IC. 
!
! Version 4.0 3/6/2018 (Batchelor) 
!         Eliminated reference to cur_cql_file and cur_dql_file since they are not 
!         used here. Also generally cleaned up.  Now requires 4 commandline arguments.
...
IF (TRIM(mode) == 'INIT') THEN
    ...
    !--------------------------------------------------------------------------
        ...
        ELSE          
             ...
             print*, 'model_RF_IC.f90: I give up'
             call exit(1)
...
IF (TRIM(mode) == 'STEP') THEN
    ...
    WRITE (*,*) "model_RF_IC: Stored Partial RF Plasma State"    
