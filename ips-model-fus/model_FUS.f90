...
!------------------------------------------------------------------------------------
    ...
    OPEN (unit=21, file = input_namelist_file, status = 'old',   &
         action='read', form = 'formatted', iostat = ierr)
            IF (ierr /= 0 ) THEN
                CALL SWIM_error ('open', 'model_FUS.f90',TRIM(input_namelist_file))
                ierr = istat
...
IF (TRIM(mode) == 'INIT') THEN
    ...
    !---------------------------------------------------------------------------------
       ...
       OPEN (unit=21, file=TRIM(internal_state_file), status='unknown', &
            ...
            IF (istat /= 0 ) THEN
                CALL SWIM_error ('open', 'model_FUS.f90',TRIM(internal_state_file))
                ierr = istat
...
IF (TRIM(mode) == 'STEP') THEN
    ...
    !---------------------------------------------------------------------------------
       ...
       OPEN (unit=21, file=TRIM(internal_state_file), status='old', &
        ...
        WRITE (*,*) 'Call ', n_step_calls, ' to model_FUS step'
	...
	WRITE (*,*) "model_FUS: Stored Partial Plasma State"    
